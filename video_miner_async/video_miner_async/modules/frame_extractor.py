...
Supports concurrent extraction across videos.
"""
...
class FrameExtractor:
    ...
    def extract_batch(
        self,
        video_paths: list[tuple[Path, str]],  # (video_path, video_id)
        ...
        Args:
            video_paths: List of (video_path, video_id) tuples
            show_progress: Show progress bar
            on_complete: Optional callback called after each video with ExtractionResult
            max_workers: Maximum concurrent extractions (default: 4)
            
        Returns:
            List of ExtractionResult for each video
        """
        if not video_paths:
            return []
        ...
        # Shared frame counter for progress heartbeat: [count, lock]
        frame_counter = [0, threading.Lock()]
        
        # Track which videos are currently processing
        if show_progress:
            tqdm.write(f"  Starting {len(video_paths)} videos with {max_workers} workers...")
        
        # Concurrent extraction with thread pool
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all extraction tasks with shared counter
            futures = {
                executor.submit(
                    self.extract_video,
                    video_path,
                    video_id,
                    save_frames=True,
                    show_progress=False,  # Disable inner progress for concurrent
                    frame_counter=frame_counter,
                ): video_id
                for video_path, video_id in video_paths
            }
            ...
            for future in iterator:
                ...
                try:
                    ...
                    # Log completion with tqdm.write
                    if show_progress:
                        status = "✓" if result.success else "✗"
                        tqdm.write(f"  {status} {video_id}: {result.frame_count} frames")
                    
                    # Update progress bar with frame count
                    if pbar:
                        pbar.update(1)
                        with frame_counter[1]:
                            pbar.set_postfix(frames=frame_counter[0], refresh=True)
        ...
        logger.info(f"Extraction complete: {successful}/{len(results)} videos, {total_frames} frames")
