...
class Sam3MultiClassPredictorBatch(Sam3MultiClassPredictorFast):
    ...
    Constraint: all images are resized to self.resolution (1008×1008) internally,
    ...
    def set_images(
        ...
        All images are resized to self.resolution (1008×1008) and stacked into
    ...
    def predict_batch(
        ...
        # unsqueeze+expand+reshape creates contiguous copies (expand on size-1
        # dim → non-contiguous, reshape then copies).  Safe for encoder which
        # may modify list elements in-place.
        BN = B * N
        batched_img_feats = [
        ...
        # repeat tiles along dim-1: [cls0..clsN-1, cls0..clsN-1, ...] B times
        # This matches the image feature ordering where position i*N+j
        # corresponds to image i, class j.
        ...
        with torch.autocast("cuda", dtype=torch.float16, enabled=self.use_fp16):
            ...
            # --- Scoring (batched — DotProductScoring handles bs=B*N) ---
        ...
        if dec_presence_out is not None:
            # dec_presence_out: (layers, 1, B*N) → last layer → (1, B*N)
