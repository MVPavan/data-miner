"""
Youtu-VL-4B image classifier for object presence filtering.
...
Usage:
    CUDA_VISIBLE_DEVICES=0 python -m data_miner.models.youtu_vl_classify
"""
...
class YoutuVLClassifier:
    def __init__(
        self,
        model_id: str = "tencent/Youtu-VL-4B-Instruct",
    ...
    def load_model(self):
        ...
        print("Youtu-VL loaded")

    @torch.no_grad()
    def _query(self, image_path: str, question: str) -> bool | None:
        """Query the model with an image and yes/no question.

        Args:
            image_path: Path string to the image file (model handles reading internally).
            question: Yes/no question to ask about the image.
        """
        messages = [
    ...
    def classify_image(self, image_path: str | Path) -> dict:
        ...
        if self.filter_non_real:
            result["is_real"] = self._query(img_path_str, REALITY_QUERY)
        ...
        for obj in self.object_classes:
            ...
            result["objects"][obj] = self._query(img_path_str, query)
    ...
    def process_folder(self, input_dir: str | Path, output_dir: str | Path):
        ...
        with open(output_dir / "summary.json", "w") as f:
            json.dump(
                {"model_id": self.model_id, "object_classes": self.object_classes, "counts": counts},
                f,
                indent=2,
            )
...
if __name__ == "__main__":
    input_folder = Path(
        "/swdfs_mnt/swshared/data_miner_output/projects/forklift_palletjack_v1/frames_dedup_v2_cls"
    )
    output_folder = Path(
        "/swdfs_mnt/swshared/data_miner_output/projects/forklift_palletjack_v1/yvl_filter_cls"
    )

    classifier = YoutuVLClassifier(
