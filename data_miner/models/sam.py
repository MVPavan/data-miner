...
class SAMHelper:
    ...
    def detect(
        ...
        for cls_idx, text_prompt in enumerate(classes):
            ...
            for box, score in zip(results.get("boxes", []), results.get("scores", [])):
                ...
                all_scores.append(float(score.cpu()) if hasattr(score, 'cpu') else float(score))
    ...
    def segment_with_text(
        ...
        return {
            "masks": [m.cpu().numpy() if hasattr(m, 'cpu') else m for m in results.get("masks", [])],
            "boxes": [b.cpu().numpy() if hasattr(b, 'cpu') else b for b in results.get("boxes", [])],
            "scores": [float(s.cpu()) if hasattr(s, 'cpu') else float(s) for s in results.get("scores", [])],
    ...
    def process_folder(
        ...
        for image_file in tqdm(image_files, desc="Processing"):
            try:
                ...
                if save_masks:
                    result = self.segment_with_text(image_file, self.detection_class[0], threshold)
                    if result["masks"]:
                        np.savez_compressed(
...
if __name__ == "__main__":
    ...
    input_folder = Path("/data/pavan/tycoai/project_helpers/data_miner/output/projects/delivery_pov_v1/frames_filtered_v2_dedup")
    output_folder = Path("/data/pavan/tycoai/project_helpers/data_miner/output/projects/delivery_pov_v1/sam3/frames_filtered_v2_dedup")
