"""
Qwen3-VL image classifier for object presence filtering.
...
Usage:
    CUDA_VISIBLE_DEVICES=0,1 python -m data_miner.models.qwen3_vl_classify
"""
...
class Qwen3VLClassifier:
    ...
    def load_model(self):
        ...
        print(f"Loading {self.model_id} | GPUs: {torch.cuda.device_count()}")
        ...
        if hasattr(self.model, "hf_device_map"):
            print(f"Devices: {set(self.model.hf_device_map.values())}")
        print("Qwen3-VL loaded")

    @torch.no_grad()
    def _query(self, image: Image.Image, question: str) -> bool | None:
        messages = [
            {
                "role": "user",
                "content": [
    ...
    def classify_image(self, image_path: str | Path) -> dict:
        ...
        if self.filter_non_real:
            result["is_real"] = self._query(image, REALITY_QUERY)
        ...
        for obj in self.object_classes:
            ...
            result["objects"][obj] = self._query(image, query)
...
if __name__ == "__main__":
    ...
    output_folder = Path(
        "/data/pavan/tycoai/project_helpers/data_miner/output/projects/delivery_pov_v1/qwen3vl_classified"
    )
