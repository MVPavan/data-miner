...
class GroundingDINOHelper:
    ...
    def load_model(self):
        ...
        self.model = AutoModelForZeroShotObjectDetection.from_pretrained(
            self.model_id,
            torch_dtype=torch.float32 if self.device != "cpu" else torch.float32,
        )
    ...
    def detect(
        ...
        boxes_np = np.array([b.tolist() for b in boxes])  # xyxy format
        scores_np = np.array([float(s) for s in scores])
        class_ids_np = create_class_ids(len(boxes))
            ...
            try:
                ...
                with open(txt_path, "w") as f:
                    for det in detections:
                        ...
                        y_center = y_min + h / 2
                        f.write(
                            f"{cls_id} {x_center:.6f} {y_center:.6f} {w:.6f} {h:.6f} {score:.6f}\n"
                        )
...
if __name__ == "__main__":
    ...
    output_folder = Path(
        "/data/pavan/tycoai/project_helpers/data_miner/output/projects/delivery_pov_v1/gdino/frames_filtered_v2_dedup"
    )
    ...
    # Process folder
    gdino.process_folder(input_folder, output_folder, threshold=0.1)
