...
_REFINEMENT_SCHEMA = RefinementProposal.model_json_schema()
...
strategy must be one of: "sam_points", "sam_box", "repropose_text".
"""


def build_refinement_agent(
    config: AutoAnnotationV2Config,
) -> Agent[RefinementDeps, str]:
    """Create the refinement proposal agent. Returns raw JSON string."""
    vlm_model = _build_vlm_model(config.vlm)

    agent: Agent[RefinementDeps, str] = Agent(
        vlm_model,
    ...
    def refinement_instructions(ctx) -> str:
        ...
        return _REFINEMENT_INSTRUCTIONS.format(
            ...
            schema=json.dumps(_REFINEMENT_SCHEMA, indent=2),
        )

    return agent


def parse_refinement_proposal(raw: str) -> RefinementProposal:
    """Parse raw JSON string from refinement agent into RefinementProposal."""
    return RefinementProposal.model_validate(_extract_json(raw))
